/**
 * IAM Ledger API — port 8000. CommonJS for maximum Windows/Node compatibility.
 * Run: node server.js
 */
  ...
  return {
    ...
    trades: [
      ...
      },
      {
        ...
        executedAt: new Date(now - 2.5 * 3600_000).toISOString(),
...
function loadStore() {
  ...
  if (!fs.existsSync(LEDGER_FILE)) {
    ...
    fs.writeFileSync(LEDGER_FILE, JSON.stringify(fresh, null, 2));
    return fresh;
  }
  try {
...
function saveStore(snapshot) {
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
  fs.writeFileSync(LEDGER_FILE, JSON.stringify(snapshot, null, 2));
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (LEDGER_PATHS.has(route) && method === "GET") {
    json(res, 200, buildPayload(loadStore()));
    return;
  }

  if (LEDGER_PATHS.has(route) && method === "POST") {
    const chunks = [];
...
server.on("error", (err) => {
  ...
  if (err.code === "EADDRINUSE") {
    ...
    console.error("  Close the other program or run: taskkill /F /IM node.exe");
  }
  console.error("");
  process.exit(1);
});
...
server.listen(PORT, HOST, () => {
  console.log("");
  console.log("  ========================================");
  console.log("  IAM LEDGER RUNNING");
  console.log("  ========================================");
  console.log("");
  console.log("  http://127.0.0.1:8000/health");
  console.log("  http://127.0.0.1:8000/api/ledger");
  console.log("");
  console.log("  DO NOT CLOSE THIS WINDOW");
  console.log("  Then refresh Investment Management");
  console.log("");
});
